package com.hwq.fundament.JavaDataStructure.avl;
...
//        int[] arr = {4, 3, 6, 5, 7, 8};
        ...
        System.out.println("根的节点左子结点=" + avlTree.getRoot().left);
        System.out.println("根的节点右子结点=" + avlTree.getRoot().right);
    }
}
...
class AVLTree {
    ...
    public void add(Node node) {
        if (root == null) {
            root = node;//如果node为空则直接让root指向node
        } else {
            root.add(node);
        }
    }
...
class Node {
    ...
    public void add(Node node) {
        ...
        //当添加完一个结点后，如果:(右子树的高度-左子树的高度的) > 1 左旋转
